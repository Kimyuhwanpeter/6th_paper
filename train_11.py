...
FLAGS = easydict.EasyDict({"img_size": 512,
                           ...
                           "tr_label_path": "D:/[1]DB/[5]4th_paper_DB/Fruit/apple_pear/augment_label/",
                           
                           "tr_image_path": "D:/[1]DB/[5]4th_paper_DB/Fruit/apple_pear/augment_train/",
...
def cal_loss(model, images, labels, object_buf):
    ...
    with tf.GradientTape() as tape:
        ...
        b_logits, o_logits = run_model(model, images, True)
        
        b_logits = tf.reshape(b_logits, [-1, ])
        o_logits = tf.reshape(o_logits, [-1, ])

        background_indices = tf.squeeze(tf.where(batch_labels == 0), -1)
        b_labels = tf.gather(batch_labels, background_indices)
        b_logits = tf.gather(b_logits, background_indices)
        
        object_indices = tf.squeeze(tf.where(batch_labels == 1), -1)
        o_labels = tf.gather(batch_labels, object_indices)
        o_logits = tf.gather(o_logits, object_indices)

        background_loss = tf.keras.losses.BinaryCrossentropy(from_logits=True)(b_labels, b_logits)
        object_loss = tf.keras.losses.BinaryCrossentropy(from_logits=True)(o_labels, o_logits)
    ...
    if FLAGS.train:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            for step in range(tr_idx):
                ...
                if count % 100 == 0:
                    b_output, o_output = run_model(model, batch_images, False)
                    for i in range(FLAGS.batch_size):
                        label = tf.cast(batch_labels[i, :, :, 0], tf.int32).numpy()
                        b_output = tf.where(tf.nn.sigmoid(b_output[i, :, :, 0]) >= 0.5, 1, 0)
                        background_indices = tf.where(b_output == 0)
                        o_output = tf.where(tf.nn.sigmoid(o_output[i, :, :, 0]) >= 0.5, 1, 0).numpy()
                        o_output[background_indices] = 0
                        final_output = o_output
            ...
            for i in range(tr_idx):
                ...
                for j in range(FLAGS.batch_size):
                    ...
                    b_output, o_output = run_model(model, batch_image, False)
                    b_output = tf.where(tf.nn.sigmoid(b_output[0, :, :, 0]) >= 0.5, 1, 0)
                    background_indices = tf.where(b_output == 0)
                    o_output = tf.where(tf.nn.sigmoid(o_output) >= 0.5, 1, 0).numpy()
                    o_output[background_indices] = 0
                    final_output = o_output
