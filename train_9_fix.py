...
def cal_loss(model, model2, images, labels, object_buf):
    ...
    with tf.GradientTape() as tape: # Edge loss functions for deep-learning depth-map
        first_output = run_model(model, images, True)
        first_output = tf.nn.sigmoid(first_output)
        
        batch_labels = tf.where(labels == 1, 0.9, 0.1)

        pred_edge = tf.image.sobel_edges(first_output)
        pred_edge_y = pred_edge[:, :, :, :, 0]
        pred_edge_y = tf.reshape(pred_edge_y, [-1, ])
        pred_edge_x = pred_edge[:, :, :, :, 1]
        pred_edge_x = tf.reshape(pred_edge_x, [-1, ])

        label_edge = tf.image.sobel_edges(batch_labels)
        label_edge_y = label_edge[:, :, :, :, 0]
        label_edge_y = tf.reshape(label_edge_y, [-1, ])
        label_edge_x = label_edge[:, :, :, :, 1]
        label_edge_x = tf.reshape(label_edge_x, [-1, ])

        total_loss = tf.reduce_mean(tf.math.abs(pred_edge_y - label_edge_y) + tf.math.abs(pred_edge_x - label_edge_x))

    grads = tape.gradient(total_loss, model.trainable_variables)
    optim.apply_gradients(zip(grads, model.trainable_variables))

    with tf.GradientTape() as tape2: # object and background loss (2-channels)
        second_output = run_model(model2, images * first_output, True)
...
def main():
    ...
    model = multi_region_class(input_shape=(FLAGS.img_size, FLAGS.img_size, 3), nclasses=1)
    model.summary()

    model2 = multi_region_class(input_shape=(FLAGS.img_size, FLAGS.img_size, 3), nclasses=2)
    model2.summary()
