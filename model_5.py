...
def attention_ASPP(tensor, filters):
    ...
    y_1 = DepthwiseConv2D(kernel_size=1, dilation_rate=1, padding='same', use_bias=False)(tensor)
    y_1 = BatchNormalization()(y_1)
    y_1 = Activation('relu')(y_1)

    y_6 = DepthwiseConv2D(kernel_size=3, dilation_rate=6, padding='same', use_bias=False)(tensor)
    y_6 = BatchNormalization()(y_6)
    y_6 = Activation('relu')(y_6)

    y_12 = DepthwiseConv2D(kernel_size=3, dilation_rate=12, padding='same', use_bias=False)(tensor)
    y_12 = BatchNormalization()(y_12)
    y_12 = Activation('relu')(y_12)
...
def modified_model(input_shape=(512, 512, 3), classes=2):
    ...
    h_att_1 = tf.keras.layers.Conv2D(filters=64, kernel_size=3, padding="same", use_bias=False)(h)
    h_att_1 = tf.keras.layers.BatchNormalization()(h_att_1)
    h_att_1 = tf.keras.layers.ReLU()(h_att_1)

    h_1 = tf.keras.layers.Conv2D(filters=64, kernel_size=3, padding="same", use_bias=False)(h_att_1)
    ...
    h_att_2 = tf.keras.layers.Conv2D(filters=128, kernel_size=3, padding="same", use_bias=False)(h)
    h_att_2 = tf.keras.layers.BatchNormalization()(h_att_2)
    h_att_2 = tf.keras.layers.ReLU()(h_att_2) + h

    h_2 = tf.keras.layers.Conv2D(filters=128, kernel_size=3, padding="same", use_bias=False)(h_att_2)
    ...
    h_att_3 = tf.keras.layers.Conv2D(filters=256, kernel_size=3, padding="same", use_bias=False)(h)
    h_att_3 = tf.keras.layers.BatchNormalization()(h_att_3)
    h_att_3 = tf.keras.layers.ReLU()(h_att_3) + h
    
    h_3 = tf.keras.layers.Conv2D(filters=256, kernel_size=3, padding="same", use_bias=False)(h_att_3)
    ...
    #decoder_2 = tf.keras.layers.Conv2D(filters=256, kernel_size=3, padding="same", use_bias=False)(decoder_2)
    #decoder_2 = tf.keras.layers.BatchNormalization()(decoder_2)
    #decoder_2 = tf.keras.layers.ReLU()(decoder_2)  # 이렇게해도 되는건가???
