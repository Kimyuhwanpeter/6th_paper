...
def cal_loss(model, model2, images, labels, object_buf):
    ...
    with tf.GradientTape() as tape2: # object and background loss (2-channels)
        ...
        background_loss_4 = false_dice_loss(background_labels, 1 - background_logits_4)
        object_loss_4 = true_dice_loss(object_labels, object_logits_4)
        
        background_loss_3 = false_dice_loss(background_labels, 1 - background_logits_3)
        object_loss_3 = true_dice_loss(object_labels, object_logits_3)
    ...
    if FLAGS.train:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            for step in range(tr_idx):
                ...
                if count % 100 == 0:
                    _, _, _, _, first_output = run_model(model, batch_images, False)
                    _, _, _, _, second_output = run_model(model2, batch_images * tf.nn.sigmoid(first_output), False)
                    for i in range(FLAGS.batch_size):
            ...
            for i in range(tr_idx):
                ...
                for j in range(FLAGS.batch_size):
                    ...
                    _, _, _, _, first_output = run_model(model, batch_image, False)
                    _, _, _, _, second_output = run_model(model2, batch_image * tf.nn.sigmoid(first_output), False)
